...
Status as of 2026-09-03 (rev. 2 — product surface corrected to **web app**,
demos reframed as runtime input). The implementation plan
(`docs/plans/2026-09-03-cs2-anti-strat-implementation.md`) is executable
end-to-end without any of these; each item unblocks the specific thing
listed.
...
`game/csgo/pak01_dir.vpk`) here — do **not** copy `pak01_dir.vpk` into the
...
## N3 — Map VPKs (place now if convenient) + refresh on game updates

Placement: `maps/<map>/<map>.vpk` — e.g. `maps/de_mirage/de_mirage.vpk`.
Source: `...\steamapps\common\Counter-Strike Global Offensive\game\csgo\
maps\de_*.vpk`. The `de_<map>_vanity.vpk` files are **not needed**
...
re-supply that `de_<map>.vpk` once.

## N4 — (Optional, whenever) multi-demo corpora per opponent

**Placement (if providing now):** drop files anywhere under `demos/` —
flat or in any subfolders you like; the pipeline ignores folder layout
...
`.venv\Scripts\python.exe scratch\scan_corpus.py` — prints map, rounds, and
rounds-of-signal per (team, map), and writes
`scratch/corpus_inventory.json`.

**You do not need to send demos to build anything** — demos are what users
